...
/**
 * Retrieves an AI response from the offline cache if available.
 *
 * @template T
 * @param {string} key - Cache key generated by {@link createCacheKey}.
 * @returns {Promise<T|null>} The cached value or `null` when missing or expired.
 */
const getCachedAIValue = async <T>(key: string): Promise<T | null> =>
  coffeeOfflineManager.getItem<T>(key);

/**
 * Persists an AI response to the offline cache with a configurable priority.
 *
 * @param {string} key - Unique cache key created via {@link createCacheKey}.
 * @param {any} value - Serializable payload to store in the cache.
 * @param {number} [priority=6] - Cache priority used by {@link coffeeOfflineManager}; higher prioritizes retention.
 * @returns {Promise<void>} Promise that resolves after cache write completes or logs a warning on failure.
 */
const setCachedAIValue = async (key: string, value: any, priority = 6): Promise<void> => {
  try {
